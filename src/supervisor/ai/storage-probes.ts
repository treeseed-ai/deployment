...
function classifyProviderError(error){const message=String(error?.message??'').toLowerCase();for(const [reason,pattern] of [['session-token',/token|jwt/],['signature',/signature|credential/],['checksum',/checksum|digest/],['request-header',/header/],['permission',/permission|action|scope/]])if(pattern.test(message)){diagnostics.providerReason=reason;break;}}
