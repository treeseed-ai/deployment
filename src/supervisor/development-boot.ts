...
type Command = (command: string, args: string[]) => unknown;
...
export function resumeDevelopmentAtBoot(sessionId: string, command: Command) {
	...
	const account = String(command('/usr/bin/getent', ['passwd', String(owners[0])])).trim().split(':');
	...
	const load = String(command('/usr/bin/systemctl', ['show', unit, '--property=LoadState', '--value'])).trim();
	if (load === 'loaded') {
		const state = String(command('/usr/bin/systemctl', ['show', unit, '--property=ActiveState', '--value'])).trim();
