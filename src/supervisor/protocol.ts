...
const supervisorOperationUnion = z.discriminatedUnion('operation', [
	...
	z.object({ operation: z.literal('component.configure'), componentId: z.string().regex(/^[a-z][a-z0-9.-]+$/u), connectionEnvironment: z.record(z.string().regex(/^[A-Z][A-Z0-9_]{0,127}$/u), z.string().max(16_384)), secretFileIds: z.array(z.string().regex(/^[a-z0-9][a-z0-9._-]{0,127}$/u)).max(128).optional(), optionalSecretEnvironment: z.array(z.string().regex(/^[A-Z][A-Z0-9_]{0,127}$/u)).max(128).optional(), sandboxGuestImageDigest: z.string().regex(/^sha256:[a-f0-9]{64}$/u).optional() }).strict(),
