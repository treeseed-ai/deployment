...
export const supervisorOperationSchema = z.discriminatedUnion('operation', [
	...
	z.object({ operation: z.literal('development.environment'), componentId: z.string().regex(/^[a-z][a-z0-9.-]+$/u), secretRefs: z.record(z.string().regex(/^[A-Z][A-Z0-9_]{0,127}$/u), z.string().regex(/^[a-z0-9][a-z0-9._-]{0,127}$/u)) }).strict(),
