...
export async function withPostgresSourceCopy<T>(staged: Awaited<ReturnType<typeof stagePostgresBackup>>,
  ...
  try {
    ...
    const lifecycle = z.object({ lifecycleStage: z.string().regex(/^[a-z-]{1,64}$/u).optional(), processReason: z.enum(['extension-owner', 'permission-denied', 'duplicate-object', 'unsupported-parameter', 'unsupported-archive', 'connection', 'timeout', 'unknown']).optional(), locations: z.array(z.string().regex(/^src\/[a-zA-Z0-9_./-]+\.[jt]s:\d+:\d+$/u)).max(6) }).safeParse(error && typeof error === 'object' && 'diagnostic' in error ? error.diagnostic : undefined);
    const diagnostic = { stage, phase, locations, causeStage: causeStage.success ? causeStage.data.stage : undefined, lifecycle: lifecycle.success ? lifecycle.data : undefined };
