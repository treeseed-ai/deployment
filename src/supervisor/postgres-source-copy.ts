...
export async function withPostgresSourceCopy<T>(staged: Awaited<ReturnType<typeof stagePostgresBackup>>,
  ...
  try {
    ...
    const lifecycle = z.object({ lifecycleStage: z.string().regex(/^[a-z-]{1,64}$/u).optional(), locations: z.array(z.string().regex(/^src\/[a-zA-Z0-9_./-]+\.[jt]s:\d+:\d+$/u)).max(6) }).safeParse(error && typeof error === 'object' && 'diagnostic' in error ? error.diagnostic : undefined);
