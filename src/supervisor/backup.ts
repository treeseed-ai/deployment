...
import { assertBackupCoverage, assertBackupStatePaths, requiredBackupState } from './backup-coverage.js';
...
export async function inspectGenerationBackup(generation: number, options: { backupRoot?: string; key?: Buffer } = {}) {
	...
	try {
		...
		const coverage = assertBackupCoverage(state.configuration, state.components, entries);
...
export async function restoreGenerationBackup(generation: number) {
	// Authenticate and validate complete coverage before any extraction or mutation.
	const inspected = await inspectGenerationBackup(generation), { path, sha256 } = checkedArchive(generation);
	if (sha256 !== inspected.sha256) throw new Error('Recovery archive changed after inspection.');
	const key = loadKey();
	const child = spawn('/usr/bin/tar', ['--extract', '--gzip', '--file', '-', '--directory', '/', '--numeric-owner', '--no-overwrite-dir'], { stdio: ['pipe', 'ignore', 'pipe'], env: { PATH: '/usr/sbin:/usr/bin:/sbin:/bin' } });
	child.stderr.resume();
	try {
