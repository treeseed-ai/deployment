...
const defaults: SandboxBrokerConfiguration = {
	...
	trustedProvidersPath: '/etc/treeseed/sandbox/providers.json', modelGateway: { upstreamBaseUrl: 'https://api.openai.com', authenticationMode: 'api-key', credentialFile: '/run/credentials/model-provider-auth', allowedProviders: [], allowedModels: [] },
...
export function loadSandboxBrokerConfiguration(path = '/etc/treeseed/sandbox/broker.json') {
	try {
		...
		return credentials ? { ...parsed, relay: { ...parsed.relay, privateKeyFile: resolve(credentials, 'relay-tls-key') }, modelGateway: { ...parsed.modelGateway, credentialFile: resolve(credentials, 'model-provider-auth') } } : parsed;
	}
	catch (error) {
