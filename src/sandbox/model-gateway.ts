...
export async function proxyModelRequest(input: { request: IncomingMessage; response: ServerResponse; configuration: SandboxBrokerConfiguration; policy: SandboxAssignment['modelPolicy'] }) {
	...
	const credential = readFileSync(input.configuration.modelGateway.credentialFile, 'utf8').trim(); if (credential.length < 20) throw new Error('Model gateway credential is unavailable.');
	const upstream = await fetch(`${input.configuration.modelGateway.upstreamBaseUrl}/v1/responses`, { method: 'POST', headers: { authorization: `Bearer ${credential}`, 'content-type': 'application/json', accept: input.request.headers.accept ?? 'application/json' }, body });
