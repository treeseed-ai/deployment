...
export const sandboxBrokerConfigurationSchema = z.object({
	...
	modelGateway: z.object({ upstreamBaseUrl: z.string().url().startsWith('https://'), credentialFile: z.string().startsWith('/run/credentials/'), allowedProviders: z.array(z.string().min(1)), allowedModels: z.array(z.string().min(1)) }).strict(),
	guestImages: z.array(z.object({ image: z.string().min(1), digest: z.string().regex(/^sha256:[a-f0-9]{64}$/u), profiles: z.array(z.enum(['read', 'unit', 'integration', 'platform', 'connected'])).min(1) }).strict()),
}).strict();
