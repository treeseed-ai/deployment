...
export const sandboxBrokerConfigurationSchema = z.object({
	...
	modelGateway: z.object({ upstreamBaseUrl: z.string().url().startsWith('https://'), authenticationMode: z.enum(['api-key', 'codex-subscription']), credentialFile: z.string().refine((path) => path.startsWith('/run/credentials/') || path.startsWith('/var/lib/treeseed/agent/execution-credentials/'), 'Model credentials must use sealed runtime custody or the encrypted provider volume.'), allowedProviders: z.array(z.string().min(1)), allowedModels: z.array(z.string().min(1)) }).strict().optional(),
