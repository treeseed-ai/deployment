...
function renderVariables(plan: HostedTopologyPlan | AuthorizedHostedTopologyPlan) {
	...
	for (const action of plan.actions) {
		...
		if (action.provider !== 'cloudflare' && context.config.deploymentEnvironment !== plan.environment) throw new Error(`${action.provider} connection ${connection.connectionRef} is not bound to the ${plan.environment} deployment environment.`);
