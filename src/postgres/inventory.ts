...
export async function inspectPostgresAllocations(serverId: string, session: PostgresInspectionSession): Promise<PostgresInventory> {
  try {
    ...
    const databases = await session.query(`SELECT d.datname AS name, r.rolname AS owner,
    ...
    const roles = await session.query(`SELECT rolname AS name, rolsuper AS superuser,
      ...
      shobj_description(oid, 'pg_authid') AS marker FROM pg_roles ORDER BY rolname`);
    ...
    const result: PostgresInventory = {
      ...
      databases: databases.rows.map(row => ({ name: requiredString(row.name), owner: requiredString(row.owner), allocationId: allocationMarker(row.marker) })),
      roles: roles.rows.map(row => ({ name: requiredString(row.name), allocationId: allocationMarker(row.marker),
