...
export async function reconcile(track?: 'stable' | 'development') {
	...
	for (const component of configurationChanged ? accepted.components : changed) await requestSupervisor({ operation: 'component.configure', componentId: component.componentId });
