...
export async function hostDoctor(verifyPlan: () => unknown, aliases: readonly string[], postgresEnabled = false, workloadsStopped = false) {
	const checks = [
	...
	if (!workloadsStopped) checks.push({ id: 'edge-tls', ok: await edgeReadiness(aliases) });
