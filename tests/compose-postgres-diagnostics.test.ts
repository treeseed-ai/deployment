...
it('returns safe PostgreSQL startup reasons through fixed bounded log capture', () => {
  const command = vi.fn((_exe: string, args: readonly string[]) => args[0] === 'ps' ? 'a'.repeat(64) : args.includes('{{.State.StartedAt}}') ? '2026-09-10T01:00:00Z' : 'postgres\texited\tunhealthy\t1');
  ...
  expect(capture).toHaveBeenCalledWith('/usr/bin/docker', ['logs', '--since', '2026-09-10T01:00:00Z', '--tail', '80', 'a'.repeat(64)], '');
});
it('does not report stale startup failures after the current service becomes healthy', () => {
  const command = vi.fn((_exe: string, args: readonly string[]) => args[0] === 'ps' ? 'c'.repeat(64) : 'postgres\trunning\thealthy\t0');
  const capture = vi.fn(() => 'FATAL: earlier failure');
  expect(composeFailureDiagnostics('postgres', 'treeseed-postgres', command, capture)).toEqual([
