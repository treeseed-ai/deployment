...
describe('unified host manager foundation', () => {
	...
	it('accepts only bounded host commands and fixed configuration or enrollment mutations', () => {
		...
		expect(supervisorOperationSchema.parse({ operation: 'component.configure', release: '1.0.0', componentId: 'api', connectionEnvironment: {} })).toEqual({ operation: 'component.configure', release: '1.0.0', componentId: 'api', connectionEnvironment: {} });
		expect(supervisorOperationSchema.parse({ operation: 'provider.enroll', connectionId: 'primary', controlPlaneUrl: 'https://api.example.test', controlPlaneAudience: 'https://api.example.test', registrationSecretId: 'provider-registration', offer: { maxConcurrentRunners: 4, capabilities: ['agent-execution'] }, files: ['agent/release/compose.yml'], projectName: 'treeseed-agent' })).toMatchObject({ operation: 'provider.enroll', connectionId: 'primary' });
		expect(() => supervisorOperationSchema.parse({ operation: 'provider.enroll', connectionId: 'primary', controlPlaneUrl: 'http://api', controlPlaneAudience: 'http://api', registrationSecretId: '../token', offer: { maxConcurrentRunners: 1, capabilities: [] }, files: ['compose.yml'], projectName: 'treeseed-agent' })).toThrow();
		expect(() => supervisorOperationSchema.parse({ operation: 'component.configure', release: '1.0.0', componentId: '../api' })).toThrow();
		const supervisor = readFileSync(resolve(process.cwd(), 'src/supervisor/execute.ts'), 'utf8');
	...
	it('creates the state roots declared by the three TreeAI components', () => {
		expect(componentStateDirectories('ai-inference')).toEqual(['data/postgres', 'data/models', 'data/inference', 'data/artifacts']);
		expect(componentStateDirectories('postgres')).toEqual(['postgres', 'postgres-os', 'lifecycle']);
	...
	it('refreshes stable metadata only when its persisted cadence is due', () => {
		...
		const state = { runtimeStopped: false, stablePaused: false, developmentPaused: false, developmentPauseOwners: [], changedAt: checkedAt.toISOString(), metadataCheckedAt: { stable: checkedAt.toISOString(), development: null } };
	...
	it('serializes API reconciliation through the same cross-process lock as systemd', () => {
		...
		expect(operations.match(/serializedReconcile\(\)/gu)?.length).toBe(3);
		expect(operations.match(/serializedReconcile\(undefined, false, \['api'\]\)/gu)?.length).toBe(1);
