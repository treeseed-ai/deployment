// Typed disposable database recovery acceptance, not a production backup backend.
import assert from 'node:assert/strict';
import { execFileSync } from 'node:child_process';
import { createCipheriv, createDecipheriv, randomBytes } from 'node:crypto';

export function recoverIdentityDatabase({ server, database, databaseName }: { server: string; database: string; databaseName: string }) {
  ...
  const command = (args: string[], input?: string | Buffer) => execFileSync('docker', args, {
    input, stdio: ['pipe', 'pipe', 'pipe'], timeout: 120_000, maxBuffer: 32 * 1024 * 1024,
  });
  const sql = (query: string, selected = databaseName) => command(['exec', database, 'psql', '-U', 'postgres', '-d', selected, '-At', '-v', 'ON_ERROR_STOP=1', '-c', query]);
  command(['stop', server]);
  ...
  try {
    ...
    const corrupt = Buffer.from(encrypted); corrupt[0] = corrupt[0]! ^ 1;
