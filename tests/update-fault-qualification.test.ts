...
describe('isolated update fault qualification', () => {
	...
	it('restores packages, state, services, and routes after an unhealthy activation', async () => {
		...
		expect(operations).toEqual(['apt.refresh', 'sandbox.trust-anchor.repair', 'compose.stop', 'compose.stop', 'backup.create', 'apt.install', 'component.configure', 'compose.activate', 'compose.stop', 'compose.stop', 'recovery.restore', 'apt.install', 'component.configure', 'compose.activate', 'component.configure', 'compose.activate', 'edge.apply']);
