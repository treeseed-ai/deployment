...
import { postgresTransferValiditySql } from '../src/postgres/transfer-locale.js';

function fixture() {
  const calls: string[] = [];
  const state = { owner: 'source_owner', value: '{"id":1}', unsupported: false, database: 'application', major: 16, owned: true, valid: true, constraint: 'PRIMARY KEY (id)' };
  const session: PostgresInspectionSession = { query: async sql => {
    calls.push(sql);
    if (sql === postgresTransferValiditySql) return { rows: [{ valid: state.valid }] };
    if (sql.includes('current_database()')) return { rows: [{ database: state.database, major: state.major, locale: { encoding: 'UTF8', collate: 'C', ctype: 'C', provider: 'c', version: null, locale: null } }] };
...
it.each(['owner','unsupported','database','major','owned','valid'] as const)('rejects unexpected %s and rolls back', async field => {
  const f = fixture();
  ...
  if (field === 'unsupported') f.state.unsupported = true;
    ...
    if (field === 'valid') f.state.valid = false;
