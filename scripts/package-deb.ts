...
const packages: Record<string, Definition> = {
	...
	'treeseed-edge': { architecture: 'amd64', depends: 'caddy, docker.io | docker-ce', description: 'Manager-owned TreeSeed Caddy edge and local TLS aliases', postinst: 'debian/edge/postinst', payload(stage) { unit(stage, 'treeseed-edge.service'); directory(resolve(stage, 'etc/treeseed/edge')); writeFileSync(resolve(stage, 'etc/treeseed/edge/Caddyfile'), ':443 { abort }\n'); } },
