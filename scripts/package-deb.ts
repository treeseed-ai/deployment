...
function kataRuntime(stage: string) {
	...
	if (manifest.architecture !== 'amd64' || !/^4\.[0-9]+\.[0-9]+$/u.test(manifest.version) || !/^[a-f0-9]{64}$/u.test(manifest.sha256)) throw new Error('Pinned Kata runtime manifest is invalid.');
	directory(cache); const archive = resolve(cache, basename(manifest.url));
	if (!existsSync(archive)) execFileSync('curl', ['--fail', '--location', '--silent', '--show-error', '--output', archive, manifest.url], { stdio: 'inherit' });
	if (Number(execFileSync('/usr/bin/stat', ['--format=%s', archive], { encoding: 'utf8' }).trim()) !== manifest.size) throw new Error('Pinned Kata runtime size mismatch.');
	const actual = execFileSync('/usr/bin/sha256sum', [archive], { encoding: 'utf8' }).split(/\s+/u)[0];
	if (actual !== manifest.sha256) throw new Error('Pinned Kata runtime checksum mismatch.');
	execFileSync('/usr/bin/tar', ['--extract', '--zstd', '--file', archive, '--directory', stage], { stdio: 'inherit' });
}
...
const packages: Record<string, Definition> = {
	...
	'treeseed-kata-runtime': { architecture: 'amd64', depends: 'containerd (>= 2.0), qemu-system-x86, zstd', description: 'Pinned Kata runtime-rs QEMU/KVM host runtime for TreeSeed assignment sandboxes', postinst: 'debian/kata-runtime/postinst', payload: kataRuntime },
