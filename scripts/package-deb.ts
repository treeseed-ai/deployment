...
const packages: Record<string, Definition> = {
	...
	'treeseed-manager': { architecture: 'amd64', depends: `treeseed-host-runtime (= ${deploymentVersion}), treeseed-sdk (= ${sdkPayload ? debianVersion(sdkPayload.version) : deploymentVersion}), treeseed-cli (= ${cliPayload ? debianVersion(cliPayload.version) : deploymentVersion}), util-linux`, description: 'TreeSeed host manager, reconciler, mTLS API, and fixed root supervisor', postinst: 'debian/manager/postinst', payload: managerPayload },
