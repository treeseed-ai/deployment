...
const bundle = postgresComponentBundle(version, commit);
mkdirSync('release/out', { recursive: true });
writeFileSync('release/out/postgres-component-release.json', `${JSON.stringify(bundle.component, null, 2)}\n`);
writeFileSync('release/out/postgres-compose.yml', bundle.compose);
